...
class JobcoinMixer
  attr_accessor :tracked_transactions, :house
  HOUSE_ADDRESS = "HouseAccount".freeze #Feel free to change this to whatever you like
  ...
  def add_transaction(deposit_address ,options)
    ...
    @tracked_transactions = tracked_transactions.merge(
      { deposit_address => mixer_struct } #deposit_address is what was generated by utils file
    )
  end

  def poll
    while !tracked_transactions.empty?
      tracked_transactions.each do |deposit_address, mixer_struct|
  ...
  def distribute_for_withdrawal
    ...
    until zero_amount_count == house.length
      #@house = {MixerStruct => whatever amount is left over from original amount (ex ".3")}
      puts house
  ...
  def transfer_to_house(deposit_address, mixer_struct)
    ...
    #transfers struct and total_amt to house 'ledger' for later
